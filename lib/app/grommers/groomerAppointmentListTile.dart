...
import 'package:petmet_app/app/grommers/showGroomerAppointmentPage.dart';
...
class GroomerAppointmentListTile extends StatefulWidget {
  GroomerAppointmentListTile(
  ...
  @override
  _GroomerAppointmentListTileState createState() =>
      _GroomerAppointmentListTileState();
}

class _GroomerAppointmentListTileState
    extends State<GroomerAppointmentListTile> {
  Groomer groomer;
  bool button = false;
  ...
  Future<void> _getVet() async {
    final vetTemp =
        await widget.database.getGroomer(widget.appointment.doctorId);
    if (mounted) {
      setState(() {
        groomer = vetTemp;
      });
  ...
  getPackage() {
    int index = groomer.packages.indexWhere((element) {
      return element["packageName"] == widget.appointment.packageName;
    });
    package = groomer.packages[index];
  }
                          ...
                          child: InkWell(
                            onTap: () {
                              Navigator.of(context, rootNavigator: true).push(
                                MaterialPageRoute(
                                  builder: (context) =>
                                      ShowGroomerAppointmentPage(
                                          widget.appointment, groomer),
                                ),
                              );
                            },
                            child: Text(
                      ...
                      child: Align(
                        ...
                        child: ButtonTheme(
                          ...
                          child: RaisedButton(
                            color: Color(0xFF03A300),
                            child: Text(
                              'Proceed to Pay ₹${widget.appointment.mode == "HomeVisit" ? package["cost"] + package["homeVisitCharges"] : package["cost"]}',
                              textAlign: TextAlign.center,
                              style: TextStyle(
                                  color: Color(0xFFFFFFFF), fontSize: pH(17)),
                            ),
                            onPressed: () async {
                              // ShowErrorDialog.show(context: context,title: "Payment method coming soon");
                              if (checkUserAns) {
                              ...
                              onPressed: () {
                                Navigator.of(context).push(MaterialPageRoute(
                                  builder: (context) => ShowGroomerPage(
                                    groomerData: groomer,
                                    packageName: widget.appointment.packageName,
                                  ),
                                ));
                              }),
                        ),
